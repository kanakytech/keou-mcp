...
A Model Context Protocol server that turns Claude (Code, Desktop, Cursor) into a creative production assistant. Type *"take this product photo and give me the IG launch set — square, story, reel, banners"* in chat. Claude orchestrates the workflow; KIE.AI / FAL.AI render the visuals; Keou Studio packages the delivery pack.
...
| *"Fan this out for the IG launch — square, story, reel"* | One source image fanned into a format-perfect delivery pack |
...
Visit **[studio.kanaky.xyz](https://studio.kanaky.xyz/#mcp)** and copy the guided prompt. That's it.
...
| **Keou Studio** *(optional)* | Delivery packs, brand kit, history, team | **Free** | — | [studio.kanaky.xyz](https://studio.kanaky.xyz) |
...
### Tools that use a free Keou Studio account

| Tool | What it does |
|---|---|
| `keou_pack_variants` | Fan a finished asset into format-perfect variants in parallel — IG square, story, reel, TikTok, ad creative, banners. Saves ~25h of manual reformatting per pack. |
...
> Without `KEOU_API_KEY` these tools return a clear message pointing at the free signup — no silent failures.
...
> **You:** *Now fan that out for the IG launch.*

```
keou_pack_variants (sourceGenerationId: 412, packType: "ads")
→ { packId: "pk_x9k2f8", formats: [{ ratio, generationId, taskId }, ...items] }
keou_pack_status (packId: "pk_x9k2f8")
→ { total: 8, ready: 8, done: true, items: [...URLs] }
```
...
A free Keou Studio account is what you add when single-asset workflows aren't enough:

- **Format packs** — one source image fanned into a publish-ready delivery pack
...
- [x] **v0.4** — Real `keou_pack_variants` + `keou_pack_status` calling the Keou Studio API
